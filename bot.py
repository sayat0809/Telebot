...
# В разделе с глобальными переменными добавляем:
roulette_stats = {}  # {user_id: {"won": int, "lost": int, "max_win": int, "max_bet": int}}
...
async def spin_roulette_task(user_id, chat_id, user_name, context, update):
    # Инициализируем total_win в начале функции
    total_win = 0
    ...
    # Инициализируем статистику для пользователя, если её нет
    if user_id not in roulette_stats:
        roulette_stats[user_id] = {
            "won": 0,
            "lost": 0,
            "max_win": 0,
            "max_bet": 0
    ...
    bets = user_bets.get((user_id, chat_id), {})
    current_bet = sum(bets.values())
    
    # Обновляем максимальную ставку
    if current_bet > roulette_stats[user_id]["max_bet"]:
        roulette_stats[user_id]["max_bet"] = current_bet

    # Обрабатываем выигрыши
    for bet, amount in bets.items():
        if isinstance(bet, int):  # Ставка на конкретное число
            if bet == result_number:
                win = amount * 12
                total_win += win
                roulette_stats[user_id]["won"] += win
                if win > roulette_stats[user_id]["max_win"]:
                    roulette_stats[user_id]["max_win"] = win
                result_text += f"Выиграл {win}🪙 на числе {bet}\n"
            elif result_number == 0:
                return_amount = amount // 2
                roulette_stats[user_id]["lost"] += return_amount
                result_text += f"Возврат {return_amount}🪙 на числе {bet} (выпал 0)\n"
            else:
                roulette_stats[user_id]["lost"] += amount
        elif bet in ["красное", "чёрное"]:  # Ставка на цвет
            if (bet == "красное" and result_color == "🔴") or (bet == "чёрное" and result_color == "⚫️"):
                ...
                if win > roulette_stats[user_id]["max_win"]:
                    roulette_stats[user_id]["max_win"] = win
                result_text += f"Выиграл {win}🪙 на {bet}\n"
            else:
                roulette_stats[user_id]["lost"] += amount
        elif "-" in bet:  # Ставка на диапазон
            ...
            if start <= result_number <= end:
                ...
                if win > roulette_stats[user_id]["max_win"]:
                    roulette_stats[user_id]["max_win"] = win
                result_text += f"Выиграл {win}🪙 на диапазоне {bet}\n"
            elif result_number == 0:
                return_amount = amount // 2
                roulette_stats[user_id]["lost"] += return_amount
                result_text += f"Возврат {return_amount}🪙 на диапазоне {bet} (выпал 0)\n"
            else:
                roulette_stats[user_id]["lost"] += amount

    # Начисляем выигрыш
    if total_win > 0:
        user_balances[user_id] += total_win
    ...
    user_spinning.discard((user_id, chat_id))
...
async def profile_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats = roulette_stats[user_id]
    
    # Формируем сообщение
    profile_message = (
        f"{user_name}: ♠️♥️\n"
        f"Монеты: {balance}🪙\n"
        f"Выиграно в рулетке: {stats['won']}🪙\n"
        f"Проиграно в рулетке: {stats['lost']}🪙\n" 
...
# В функции spin_roulette_task обновляем статистику (в части расчета выигрыша):
if total_win > 0:
    if bet_user_id not in roulette_stats:
        roulette_stats[bet_user_id] = {"won": 0, "lost": 0, "max_win": 0, "max_bet": 0}
    
    roulette_stats[bet_user_id]["won"] += total_win
    if total_win > roulette_stats[bet_user_id]["max_win"]:
        roulette_stats[bet_user_id]["max_win"] = total_win
else:
    roulette_stats[bet_user_id]["lost"] += amount

# Обновляем максимальную ставку
current_bet = sum(bets.values())
if current_bet > roulette_stats[bet_user_id]["max_bet"]:
    roulette_stats[bet_user_id]["max_bet"] = current_bet
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex(r'^(📋Профиль|профиль|Профиль)$'), profile_handler))
